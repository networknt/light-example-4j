...
public class BodyHandler implements MiddlewareHandler {
    static final Logger logger = LoggerFactory.getLogger(com.networknt.body.BodyHandler.class);
    ...
    private void attachFormDataBody(final HttpServerExchange exchange) throws IOException {
        ...
        if (parser != null) {
           ...
           // data = BodyConverter.convert(formData);
            exchange.putAttachment(REQUEST_BODY, formData);
        } else {
            InputStream inputStream = exchange.getInputStream();
    ...
    public void register() {
        ModuleRegistry.registerModule(com.networknt.body.BodyHandler.class.getName(), Config.getInstance().getJsonMapConfigNoCache(CONFIG_NAME), null);
    }
}
